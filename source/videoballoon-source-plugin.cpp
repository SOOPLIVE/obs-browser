...
obs_properties_t *videoballoon_source_get_properties(void *data)
{
...
void RegisterVideoBalloonSource()
{
	struct obs_source_info info = {};
	info.id = "soop_videoballoon_source";
	info.type = OBS_SOURCE_TYPE_INPUT;
	info.output_flags = OBS_SOURCE_VIDEO | OBS_SOURCE_AUDIO |
		OBS_SOURCE_CUSTOM_DRAW | OBS_SOURCE_INTERACTION |
		OBS_SOURCE_DO_NOT_DUPLICATE | OBS_SOURCE_SRGB;
	//
	info.get_properties = videoballoon_source_get_properties;
	info.get_defaults = videoballoon_source_get_defaults;
	info.icon_type = OBS_ICON_TYPE_BROWSER;

	info.get_name = [](void *) { return obs_module_text("VideoBalloon.GetName"); };
	...
	info.update = [](void *data, obs_data_t *settings) {
		static_cast<BrowserSource *>(data)->UpdateVideoBalloonSource(
			settings);
	};
	...
	info.activate = [](void *data) {
		...
		/*obs_source_set_name(bs->source,
				    obs_module_text("VideoBalloon.GetName"));*/
	};
...
void UnRegisterVideoBalloonSource() {
}

// BrowserSource
void BrowserSource::UpdateVideoBalloonSource(obs_data_t *settings)
{
	if (settings) {
		std::string url_;
		//
		...
		if (url_ == url && reroute_audio_ == reroute_audio && shutdown_ == shutdown_on_invisible)
		{
