...
const int kboSourceCount = sizeof(kbo_source_type) / sizeof(kbo_source_type[0]);

obs_properties_t* kbo_graphic_source_get_properties(void* data)
{
	...
	obs_properties_add_button(
		props, "KBOGraphicList", obs_module_text("KBOGraphicList"),
		[](obs_properties_t*, obs_property_t*, void* data) {
			BrowserSource* bs = static_cast<BrowserSource*>(data);

			QString kbo_url;
			kbo_url = QString::fromStdString(URL_GRAPHICBROAD_LIST);
			kbo_url += bs->kbo_type;

			QDesktopServices::openUrl(QUrl(kbo_url));
			return false;
		});
...
#define KBO_SOURCE_PREFIX	"soop_kbo_graphic_source_"
#define id_kbo_name_func(t)	info.id = KBO_SOURCE_PREFIX t; \
				info.get_name = [](void *) { \
					return obs_module_text("KBOGraphic.GetName." t); \
				};

void RegisterKBOGraphicSource(int idx)
{
	struct obs_source_info info = {};
	KBOSourceType type = (KBOSourceType)idx;
	switch (type) {
	case All:
		id_kbo_name_func("all");
		break;
	case Score:
		id_kbo_name_func("score");
		break;
	case Stadium:
		id_kbo_name_func("stadium");
		break;
	case Player:
		id_kbo_name_func("player");
		break;
	case Livetext:
		id_kbo_name_func("livetext");
		break;
	}
	...
	//
	info.get_properties = kbo_graphic_source_get_properties;
	info.get_defaults = kbo_graphic_source_get_defaults;
	...
	info.create = [](obs_data_t *settings,
		...
		if (bs) {
			...
			size_t pos = id.find(KBO_SOURCE_PREFIX);
			if (std::string::npos != pos) {
				bs->kbo_type = id.substr(strlen(KBO_SOURCE_PREFIX));
			}
		}
		return bs;
	};
	...
	info.update = [](void *data, obs_data_t *settings) {
		static_cast<BrowserSource *>(data)->UpdateKBOGraphicSource(
			settings);
	};
	...
	info.activate = [](void *data) {
		...
		/*obs_source_set_name(bs->source,
				    obs_module_text("KBOGraphicList"));*/
	};
...
void UnRegisterKBOGraphicSource() {
}

// BrowserSource
void BrowserSource::UpdateKBOGraphicSource(obs_data_t *settings)
{
	if (settings) {
		std::string url_;
		//
