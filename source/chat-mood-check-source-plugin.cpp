...
//
obs_properties_t* chat_mood_check_source_get_properties(void* data)
{
...
//
void RegisterChatMoodCheckSource()
{
	struct obs_source_info info = {};
	info.id = "soop_chat_source_mood_check";
	info.type = OBS_SOURCE_TYPE_INPUT;
	info.output_flags = OBS_SOURCE_VIDEO | OBS_SOURCE_AUDIO |
		OBS_SOURCE_CUSTOM_DRAW | OBS_SOURCE_INTERACTION |
		OBS_SOURCE_DO_NOT_DUPLICATE | OBS_SOURCE_SRGB;
	info.get_properties = chat_mood_check_source_get_properties;
	info.get_defaults = chat_mood_check_source_get_defaults;
	info.icon_type = OBS_ICON_TYPE_BROWSER;

	info.get_name = [](void *) { return obs_module_text("Dowoomi.GetName.ChatMoodCheck"); };
	...
	info.update = [](void *data, obs_data_t *settings) {
		static_cast<BrowserSource *>(data)->UpdateChatMoodCheckSource(
			settings);
	};
...
void UnRegisterChatMoodCheckSource() {
}

// BrowserSource
void BrowserSource::UpdateChatMoodCheckSource(obs_data_t *settings)
{
	if (settings) {
		std::string url_;
		//
